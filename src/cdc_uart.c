...
void UART_IRQHandler(void)
{
	...
	if (LPC_USART->IER & UART_IER_THREINT)
	{
		if ((g_uCOM.usbTxBusy == 0) && USB_IsConfigured(g_uCOM.hUsb)) {
			...
			//-------------- THIS CODE SHOW HOW SEND TIME ON CONSOLE VIA USB -----------------
			itoa(sec,bufor_s,10);
			itoa(min,bufor_m,10);
			itoa(hours,bufor_h,10);
			strcpy(bufor_full,"Time from RTC on request - ");

			strcat(bufor_full, (const char*)bufor_h);
			strcat(bufor_full," : ");
			strcat(bufor_full, (const char*)bufor_m);
			strcat(bufor_full," : ");
			strcat(bufor_full, (const char*)bufor_s);
			strcat(bufor_full,"\r\n\0");

			DEBUGOUT("\r\n STRING %s\r\n", bufor_full);

			count = USBD_API->hw->WriteEP(g_uCOM.hUsb, USB_CDC_IN_EP, bufor_full, 44);
...
ErrorCode_t UCOM_init(USBD_HANDLE_T hUsb, USB_CORE_DESCS_T *pDesc, USBD_API_INIT_PARAM_T *pUsbParam)
{
	...
	if (ret == LPC_OK) {
		...
		if (ret == LPC_OK) {
			...
			pCDC->line_coding.dwDTERate = 9600;
